...
def load_param_from_t7(model, in_layer_index, out_layer):
    out_layer.weight = torch.nn.Parameter(torch.Tensor(
        model['modules'][in_layer_index]['weight']))
    out_layer.bias = torch.nn.Parameter(torch.Tensor(
        model['modules'][in_layer_index]['bias']))
