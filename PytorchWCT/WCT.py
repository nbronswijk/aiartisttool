...
wct = WCT(args).cuda()
@torch.no_grad()
def styleTransfer(encoder, decoder, contentImg, styleImg):
    ...
    csF = wct.transform(cF, sF, args.alpha).cuda()
...
for i, (cImg, sImg, imname) in enumerate(loader):
    ...
    for k in range(args.num_run):
      logprinter("Processing stage 5"); cImg = styleTransfer(wct.e5, wct.d5, cImg, sImg)
      logprinter("Max GPU memory allocated: %.4f GB" % (torch.cuda.max_memory_allocated(device=0) / 1024. / 1024. / 1024.))
      logprinter("Processing stage 4"); cImg = styleTransfer(wct.e4, wct.d4, cImg, sImg)
      logprinter("Max GPU memory allocated: %.4f GB" % (torch.cuda.max_memory_allocated(device=0) / 1024. / 1024. / 1024.))
      logprinter("Processing stage 3"); cImg = styleTransfer(wct.e3, wct.d3, cImg, sImg)
      logprinter("Max GPU memory allocated: %.4f GB" % (torch.cuda.max_memory_allocated(device=0) / 1024. / 1024. / 1024.))
      logprinter("Processing stage 2"); cImg = styleTransfer(wct.e2, wct.d2, cImg, sImg)
      logprinter("Max GPU memory allocated: %.4f GB" % (torch.cuda.max_memory_allocated(device=0) / 1024. / 1024. / 1024.))
      logprinter("Processing stage 1"); cImg = styleTransfer(wct.e1, wct.d1, cImg, sImg)
      logprinter("Max GPU memory allocated: %.4f GB" % (torch.cuda.max_memory_allocated(device=0) / 1024. / 1024. / 1024.))
